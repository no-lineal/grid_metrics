...
class GridScorer(object):
  ...
  def pad_edges( self, mat, h, l ):

    npad_ij = np.ceil( np.array( h.shape ) / l )
    npad_i = int( npad_ij[0] )
    npad_j = int( npad_ij[1] )

    in_size = mat.shape
    out_size = in_size + np.array([ 2*npad_i, 2*npad_j ])

    out_mat = np.full( out_size, np.nan )

    out_mat[
        npad_i : npad_i + in_size[0],
        npad_j : npad_j + in_size[1]
    ] = mat

    return out_mat, npad_i, npad_j
  
  def calculate_cc(self, mat1):
    ...
    out_mat = np.full( (mc, nc), np.nan )

    i_size, j_size = mat2.shape
    work_mat, npad_i, npad_j = self.pad_edges( mat1, mat2, 1 ) 

    for i in range( out_mat.shape[0] ):
        for j in range( out_mat.shape[1] ):

            sub_mat = work_mat[
                npad_i + i - i_size + 1 : npad_i + i + 1, 
                npad_j + j - j_size + 1 : npad_j + j + 1
            ]

            nan_sub_mat = sub_mat * mat2

            notnan_inds = np.where( ~np.isnan( nan_sub_mat ) )
            n = notnan_inds[0].shape[0]
  ...
  def get_scores(self, rate_map, algorithm):
    ...
    if algorithm == 'sac':
      ...
      return ( scores_60[max_60_ind], scores_90[max_90_ind], self._masks[max_60_ind][1], self._masks[max_90_ind][1], sac )
    
    elif algorithm == 'cc':

      sac = self.calculate_cc(rate_map)
      rotated_sacs = self.rotated_sacs(sac, self._corr_angles)
